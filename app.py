    ...
    try:
        ...
        def classify(d):
            ...
            if dataset_type == "vaccine":
                ...
                # 🔥 STRICT 4 TYPE CLASSIFICATION ONLY

                # Research
                if any(x in d for x in ["research", "r&d", "development", "trial"]):
                    return "Research"

                # Quality Control
                elif any(x in d for x in ["control", "qc", "quality control", "calibrator", "standard"]):
                    return "Quality Control"

                # Screening
                elif any(x in d for x in ["screening", "rapid", "mass testing"]):
                    return "Screening"

                # Diagnostic (default + closest fit)
                else:
                    return "Diagnostic"
